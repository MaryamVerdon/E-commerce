...
class PaymentService {
    ...
    public function newPayment($commande)
    {
        ...
        try {
            ...
            return ['message' => $e->getMessage(), 'code' => $e->getCode(), 'data' => $e->getData()];
        }
    ...
    private function commandeToItemList($commande)
    {
        ...
        $shipping_address->setCity($adresse->getVille());
        $shipping_address->setCountryCode($this->countryToCountryCode($adresse->getPays()));
        $shipping_address->setPostalCode($adresse->getCp());
        $shipping_address->setLine1($adresse->getAdresse());
        $shipping_address->setState($adresse->getPays());
        $shipping_address->setRecipientName($commande->getClient()->getPrenom() . " " .$commande->getClient()->getNom());

        $list->setShippingAddress($shipping_address);
    ...
    private function countryToCountryCode($country){
        ...
        return array_search(strtolower($country), array_map('strtolower', $countrycodes));
    }
